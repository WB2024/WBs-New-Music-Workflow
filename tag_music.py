...
def run_tagging(music_path, config_overrides=None, log_dir=None):
    """Run Essentia analysis and tagging on a directory. For programmatic use.

    Args:
        music_path: str path to directory to process
        config_overrides: dict with settings. Values should use the same units
            as the Config class defaults (thresholds as fractions, not percent).
            Keys: dry_run, enable_genres, enable_moods, top_n_genres,
            genre_threshold, mood_threshold, genre_format,
            write_confidence_tags, overwrite_existing, verbose, workers,
            max_audio_duration, model_dir
        log_dir: directory for log file (defaults to cwd)
    ...
    config = Config()
    config.dry_run = overrides.get('dry_run', config.dry_run)
    config.enable_genres = overrides.get('enable_genres', config.enable_genres)
    config.enable_moods = overrides.get('enable_moods', config.enable_moods)
    config.top_n_genres = overrides.get('top_n_genres', config.top_n_genres)
    config.genre_threshold = overrides.get('genre_threshold', config.genre_threshold)
    config.mood_threshold = overrides.get('mood_threshold', config.mood_threshold)
    config.genre_format = overrides.get('genre_format', config.genre_format)
    config.write_confidence_tags = overrides.get('write_confidence_tags', config.write_confidence_tags)
    ...
    if log_dir:
        ...
        config.log_file = os.path.join(log_dir, log_filename)
    else:
        config.log_file = log_filename
